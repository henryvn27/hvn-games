 ...
 * checkers_win {}; trade_properties {count}; dockside_run {crates}; golf_hole {strokes,par} on an
...
(function () {
  ...
  const GAME_IDS = ['snake', 'dodger', 'memory', 'reaction', 'word', 'flappy', 'platform', 'tic', 'checkers', 'trade', 'golf', 'driftlock', '2048', 'dockside', 'invaders', 'hex-stack', 'minesweeper'];
  ...
  const definitions = [
    ...
    ['all-games', 'Full shelf', `Try all ${GAME_IDS.length} games.`, '🌟', 'played', GAME_IDS.length],
    ...
    ['word-win', 'Wordle solved', 'Solve a Wordle puzzle.', '🔓', 'wordWins', 1],
    ['word-two', 'Code whisperer', 'Solve Wordle in two guesses or fewer.', '🔤', 'wordGuesses', 2, 'atMost', 'guesses'],
    ['clicker-five', 'Power player', 'Reach 5 field-kit power in Field Station.', '🧭', 'clickerPower', 5],
    ['flyer-ten', 'Clear skies', 'Pass 10 gates in one Sky Flyer flight.', '✈', 'flyerScore', 10],
    ['platform-win', 'Platform pro', 'Reach the flag in Mini Platformer.', '🚩', 'platformWins', 1],
  ...
  const baseline = () => ({version: 1, stats: {played: [], dailySets: 0, snakeScore: 0, bossWins: 0, memoryMoves: null, reactionMs: null, wordWins: 0, wordGuesses: null, clickerPower: 1, flyerScore: 0, platformWins: 0, ticWins: 0, checkersWins: 0, tradeProperties: 0, docksideCrates: 0, invadersWave: 1, minesweeperWins: 0, golfHoles: 0, golfBestHole: null, golfRounds: 0, golfBestDelta: null}, earned: {}, equipped: {theme: 'classic', snake: 'classic', ship: 'classic', plane: 'classic', golf: 'classic'}});
  ...
  function hydrate(raw) {
    ...
    for (const [metric, max] of [['dailySets', 10000], ['snakeScore', 400], ['bossWins', 1], ['wordWins', 1], ['flyerScore', 1000000], ['platformWins', 1], ['ticWins', 1], ['checkersWins', 1], ['tradeProperties', 100], ['docksideCrates', 1000], ['invadersWave', 10000], ['minesweeperWins', 1], ['golfHoles', 1], ['golfRounds', 1]]) {
  ...
  function record(event, data = {}) {
    ...
    switch (event) {
      ...
      case 'dockside_run': if (integer(data.crates, 0, 1000)) best('docksideCrates', data.crates); break;
